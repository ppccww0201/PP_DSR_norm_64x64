...
def points_to_voxel(points,
    ...
    if reverse_index:
        voxel_num, voxels_exceed_num = _points_to_voxel_reverse_kernel(
    ...
    # File = open("/home/MASS/PythonProject/MASS_v2_1/output/voxel_num.txt", "a")
    # File.write('%f\n' % voxel_num)
    # File.close()
...
def _points_to_voxel_reverse_kernel(points,
    ...
    # reduce performance
    voxels_exceed_num = np.zeros(
        shape=(max_voxels, 1), dtype=points.dtype)
    N = points.shape[0]
    ...
    voxel_num = 0
    failed = False
    th = [np.arange(coors_range[0].item(), coors_range[3].item(), voxel_size[0].item()), np.arange(coors_range[1].item(), coors_range[4].item(), voxel_size[1].item())]
    for i in range(N):
        failed = False
        for j in range(ndim-1):
            # c = (th[j]<=points[i, j]).sum() - 1
        ...
        if voxelidx == -1:
            # 만약 pillar 좌표 a,b,c에 새로운 point가 속해졌다면, coor_to_voxelidx(500,1000)의 해당 위치(a,b,c)에 새로운 pillar index(voxelidx)가 부여된다.
            # 이 index는 최대 pillar 개수인 max_voxels로 제한된다.
            # 그리고 coors의 index 번째 열에는 (a,b,c) 좌표가 채워진다. 즉 coors의 i번째 열에 접근하면, i번째 pillar의 좌표를 알 수 있다.
            voxelidx = voxel_num
        ...
        if num < max_points:
            ...
            voxels_exceed_num[voxelidx] +=1
    return voxel_num, voxels_exceed_num
